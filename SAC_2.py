...
class Actor(nn.Module):
    ...
    def get_action(self, state_encoded):
        ...
        log_prob = normal.log_prob(z) - torch.log(2 * (1 - torch.tanh(z).pow(2)) + 1e-6)#torch.log(1 - (action/2).pow(2) + 1e-6)  # Termine di correzione standard
...
class SACAgent:
    def __init__(self, state_dim, action_dim, lr=1e-4, gamma=0.99, tau=0.05, alpha=0.2):
        self.gamma = gamma
        self.tau = tau
        self.alpha = alpha
        ...
        self.policy_optimizer = optim.Adam(list(self.policy.parameters()), lr=lr)
    ...
    def update(self, batch_size=128):
        ...
        for _ in range(40):
            ...
            policy_loss = ((self.alpha * log_probs_new) - torch.min(q1_new, q2_new) ).mean()
